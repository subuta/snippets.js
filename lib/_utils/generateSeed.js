...
export const generateJunctionSeeds = (model, modelsConfig, count = 1, opts = {}) => {
  return _.sortBy(_.times(count, (i) => generateJunctionSeed(model, modelsConfig, i, opts)), 'id')
}
...
export const generateJunctionSeed = (model, modelsConfig, offset = 0, opts = {}) => {
  ...
  faker.seed(hash(JSON.stringify(modelConfig.schema)) + offset)
  ...
  return _.transform(properties, (result, prop, key) => {
    ...
    result[key] = (function generator (prop, key) {
      ...
      return seedDataGenerator(tableName, prop, key, opts)
    })(prop, key)
  }, {})
}
...
export const generateSeeds = (modelConfig, count = 1, opts = {}) => {
  return _.sortBy(_.times(count, (i) => generateSeed(modelConfig, i, opts)), 'id')
}
...
export const generateSeed = (modelConfig, offset = 0, opts = {}) => {
  ...
  return _.transform(properties, (result, prop, key) => {
    result[key] = seedDataGenerator(tableName, prop, key, opts)
  }, {})
}

// generate seed data via faker.js
const seedDataGenerator = (tableName, prop, key, opts) => {
  const {type} = prop
  const _key = _.snakeCase(key)
  ...
  if ((_key === 'id' || _.endsWith(_key, 'id')) && type === 'number') {
    return faker.random.number()
  }
  ...
  switch (type) {
    case 'array':
      return [seedDataGenerator(tableName, prop.items, key, opts)]
    case 'object':
      return _.transform(prop.properties || {}, (result, p, k) => {
        result[k] = seedDataGenerator(tableName, p, k, opts)
      }, {})
    case 'number':
    case 'integer':
      return faker.random.number()
