...
export const generateSeeds = (modelConfig, count = 1, locale = 'en',) => {
  return _.times(count, (i) => generateSeed(modelConfig, i, locale))
}

export default function generateSeed (modelConfig, offset = 0, locale = 'en') {
  validateModel(modelConfig)

  // use hash of json as a randomness
  faker.seed(hash(JSON.stringify(modelConfig)) + offset);
