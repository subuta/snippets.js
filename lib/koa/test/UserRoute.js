...
export default (props) => {
  ...
  const sanitizeSeed = (seed) => {
    if (schema.visible) {
      return _.pick(seed, schema.visible)
    } else if (schema.hidden) {
      return _.omit(seed, schema.hidden)
    }
  }
  ...
  return build`
    ...
    test('get me should return user', async (t) => {
      ...
      ${_.map(sanitizeSeed(oldSeed), (value, key) => {
    ...
    test('put me should update user if exists', async (t) => {
      ...
      ${_.map(sanitizeSeed(seedForUpdate), (value, key) => {
        return `t.deepEqual(response.body.${key}, ${s.stringify(value)})`
      })}
    })
    ...
    test('post me should create user if not exists', async (t) => {
      ...
      ${_.map(_.omit(sanitizeSeed(seed), 'id'), (value, key) => {
        return `t.deepEqual(response.body.${key}, ${s.stringify(value)})`
      })}
    })
    ...
    test('post me should not create new user if already exists', async (t) => {
      ...
      ${_.map(sanitizeSeed(seed), (value, key) => {
        return `t.deepEqual(response.body.${key}, ${s.stringify(value)})`
      })}
    })
