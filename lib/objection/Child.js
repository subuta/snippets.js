...
export default ({model, config}, children = []) => {
  ...
  // FIXME: boolean becomes int at SQLite but not for Postgres
  // FIXME: if boolean column has int value then convert it to suitable value.
  // let {
  //   properties,
  // } = schema
  //
  // const booleanProps = _.map(_.pickBy(properties, {type: 'boolean'}), (v, k) => {
  //   return `json['${k}'] = !!parseInt(json['${k}'])`
  // })
